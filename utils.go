...
func getFunctionOffset(elfFile string, fnName string) (uint64, error) {
	// this code was taken from here:
	// https://github.com/safchain/koa/blob/master/ebpf/ebpf.go#L76
	file, err := elf.Open(elfFile)
	if err != nil {
		return 0, err
	}
	...
	if offset < bAddr {
		return 0, fmt.Errorf("Wrong symbol offset: %s", fnName)
	}
	offset -= bAddr
